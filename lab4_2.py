    ...
    for p in range(n):
        for q in range(n):
            if p != q:
                ...
                for x in range(n):
                    if x != p and x != q:
                        if ccw(data[p], data[q], data[x]) <= 0:
                            seg = False
                if seg == True:
                    hull.append([data[p][0], data[p][1], data[q][0], data[q][1]])
...
def get_convex_hull_graham(data):
    ...
    for i in range(2,n): 
      j = i
      while j>1 and (ccw(data[P[0]],data[P[j-1]],data[P[j]])<0): 
        P[j], P[j-1] = P[j-1], P[j]
        j -= 1
    hull.append(P[0])
    hull.append(P[1])
    for i in range(2,n):
      while ccw(data[hull[-2]],data[hull[-1]],data[P[i]])<=0:
        del hull[-1] 
      hull.append(P[i])
