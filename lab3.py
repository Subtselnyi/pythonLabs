...
def quick_sort(seq, low, high):
    ...
    """
    if low < high:
        pivot_location = partition(seq, low, high)
        quick_sort(seq,low, pivot_location)
        quick_sort(seq,pivot_location + 1, high)
    return seq

def randomized_partition(seq, low, high):
    """
    Процедура розбиття для методу швидкого сортування.
    Розбиває вхідний масив seq[p..r-1] на дві частини відносно опорного елементу
    наступним чином: ліва частина seq[p..q-1] містить елементи (у довільному 
    ...
    for index in xrange(low,high):
        if seq[index]<seq[high]:
            counter+=1
            newPivotIndex=newPivotIndex+1
            temp=seq[newPivotIndex]
            seq[newPivotIndex]=seq[index]
            seq[index]=temp
    temp=seq[newPivotIndex+1]
    seq[newPivotIndex+1]=seq[high]
    seq[high]=temp
...
def randomized_quick_sort(seq, low, high):
    """
    ...
    if low<high:
        ...
        p=randomized_partition(seq, low, high)
        randomized_quick_sort(seq, low,p-1)
        randomized_quick_sort(seq, p+1, high)
def test(f, data):
    """
...
"""
a = generate_data(1000, gen_type="random")
